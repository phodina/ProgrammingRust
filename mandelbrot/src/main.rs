...
///
/// Specifically, `s` should have the form <left><sep><right>, where <sep> is the character given by the `separator` argument, and <left> and  <right> are both strings that can be parsed by `T::from_str`.
///
/// If `s` has the proper form, return `Some<x,y>`. If it doesn't parse correctly and return `None`
...
///
/// If 'c' is not a member, return 'Some(i)' where 'i' is the number of the iterations it took for 'c' to leave the circle of radius wto centered on the origin. If 'c' seems to be a member (more precisely, if we reached the integration limit without being able to prove that 'c' is not a member), return 'None'
...
///
/// The `bounds` argument gives the width and height of the buffer `pixels`, which holds one greyscale pixel per byte. The `upper_left` and `lower_right` arguments specify points on the complex plane corresponding to the upper-ledt and lower-right corners of the pixel buffer.
